...
QString accountsCache::getHost(const QString& bareJid)
{
    QDomElement element = m_accountsDocument.documentElement().firstChildElement("account");
    while(!element.isNull())
    {
        if(element.firstChildElement("bareJid").text() == bareJid)
        {
            QString host = element.firstChildElement("host").text();
            return host;
        }
        element = element.nextSiblingElement("account");
    }
...
QString accountsCache::getPort(const QString& bareJid)
{
    QDomElement element = m_accountsDocument.documentElement().firstChildElement("account");
    while(!element.isNull())
    {
        if(element.firstChildElement("bareJid").text() == bareJid)
        {
            QString port = element.firstChildElement("port").text();
            return port;
        }
        element = element.nextSiblingElement("account");
    }
...
void accountsCache::addAccount(const QString& bareJid, const QString& passwd , const QString& host , const QString& port)
{
    ...
    QDomElement newHost = m_accountsDocument.createElement("host");
    newHost.appendChild(m_accountsDocument.createTextNode(host));
    newElement.appendChild(newHost);

    QDomElement newPort = m_accountsDocument.createElement("port");
    newPort.appendChild(m_accountsDocument.createTextNode(port));
    newElement.appendChild(newPort);
